...
export function KnowledgeUploader() {
  ...
  return (
    <form action={submit} className="rounded-2xl border border-border bg-card p-5">
      ...
      <p className="mt-2 text-sm text-foreground-2">Upload Markdown, text, or PDF support docs. Sources are chunked and approved for RAG.</p>
      <div className="mt-4 grid gap-3 md:grid-cols-[1fr_180px]">
        ...
        <input name="file" type="file" accept=".md,.txt,.pdf,text/*,application/pdf" className="rounded-xl border border-border bg-surface px-3 py-2 text-sm md:col-span-2" required />
      </div>
