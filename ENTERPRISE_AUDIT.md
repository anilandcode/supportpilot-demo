...
| Custom domain verification automation | PARTIAL | `supabase/migrations/010_domain_verification_tokens.sql`, `GET|POST /api/workspaces/[workspaceId]/domains`, `POST /api/workspaces/[workspaceId]/domains/[domainId]/verify`, `verifyWorkspaceDomain()`, and `npm run test:domains` add pending-domain tokens, TXT/CNAME checks, audit logging, and widget-origin activation only after verification. Background recheck scheduling and production DNS monitoring remain follow-up. |
