...
| Add CI gates for production checks | PARTIAL | `.github/workflows/ci.yml` runs typecheck, billing, rate-limit, embeddings, ingestion, integrations, retention, domains, RLS, enterprise, production-readiness, diff whitespace, and build checks on PRs and `main`, with an RLS report artifact. Playwright E2E, golden eval, and load-test gates remain follow-up. |
