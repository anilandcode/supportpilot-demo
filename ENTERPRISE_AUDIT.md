...
| Integrations, production embeddings, persistent limits, background ingestion, retention, and local runtime | PARTIAL | Production embeddings and persistent rate limits are scaffolded; `supabase/migrations/007_background_ingestion_jobs.sql`, `lib/db/ingestion.ts`, `/api/knowledge/upload`, `/api/knowledge/ingest/jobs`, and `scripts/test-ingestion.ts` add ingestion status, retry metadata, QStash worker hook, duplicate content hashes, and manual retry. Live QStash provisioning, object storage handoff, retention deletion jobs, external integrations, and load tests remain. |
