...
| --- | --- | --- |
| Supabase-first backend with Postgres, pgvector, Storage-ready data model, and RLS | PASS | `supabase/migrations/001_enterprise_supportpilot.sql` enables `vector`, creates the enterprise schema, RPC retrieval, indexes, and RLS policies. |
...
| Custom observability source of truth | PASS | `ai_runs` and `audit_logs` are created in SQL and written by `lib/db/support.ts`, `/api/chat`, draft, and decision routes. |
...
| External send/email/helpdesk integration | PARTIAL | Approval creates the canonical ticket message and audit log; no external email/helpdesk connector is included yet. |
...
| Knowledge upload for md/txt/pdf | PASS | `POST /api/knowledge/upload` parses text/PDF, chunks content, stores docs/chunks, and logs audit events. |
...
| Portfolio docs | PASS | `ROLE_ALIGNMENT.md`, `ARCHITECTURE.md`, `EVALS.md`, `SECURITY.md`, `ROADMAP.md`, README, setup, and ingest docs. |
