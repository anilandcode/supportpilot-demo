...
| Add membership update, disable, and invite revoke safeguards | PARTIAL | `PATCH|DELETE /api/workspaces/[workspaceId]/memberships/[membershipId]` supports role changes and disabling with audit logs, owner/admin role gates, and final-active-owner protection. `DELETE /api/workspaces/[workspaceId]/invitations` revokes pending invites with audit logging. `npm run test:production` covers ownerless prevention, admin owner boundaries, route authorization, and invite revocation evidence. Member-management UI and invite email delivery remain follow-up. |
