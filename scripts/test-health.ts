...
async function main() {
  ...
  checks.push([
    "health alert secret is optional but enforced when configured",
    verifyHealthAlertSecret("health-secret") && !verifyHealthAlertSecret("wrong-secret"),
    "secret guard",
  ]);

  process.env.SUPPORTPILOT_HEALTH_ALERT_WEBHOOK_URL = "https://hooks.example.test/supportpilot?token=redacted";
  const alertCalls: Array<{ url: string; body: string | null }> = [];
  const alertResult = await sendHealthAlert(productionMissing, async (url, init) => {
  ...
  checks.push([
    "health alert sends sanitized failing check payload",
    alertResult.status === "sent" &&
      alertResult.destination === "https://hooks.example.test/supportpilot" &&
      alertPayload?.service === "supportpilot" &&
      alertPayload?.status === "fail" &&
      alertPayload?.failingChecks?.some((check: { key: string }) => check.key === "supabase") &&
      !JSON.stringify(alertPayload).includes("token=redacted"),
    alertResult.status,
  ]);

  const healthyAlert = await sendHealthAlert(productionReady, async () => new Response("unexpected", { status: 500 }));
  checks.push([
    "health alert skips healthy snapshots",
    healthyAlert.status === "skipped" && healthyAlert.reason === "healthy",
    healthyAlert.status,
  ]);
